...
void main() {
  ...
  test('resume saved matches, then win human-paced 1-pointers vs a weak bot',
      () async {
    final env = _env();
    ...
    var lobbyReady = false; // wait out the login burst so saved games register
    ...
    void finish(String why) {
      ...
      // ignore: avoid_print
      print('SESSION DONE — $why (wins=$wins losses=$losses invites=$invites)');
      stallTimer?.cancel();
    ...
    void doLobby() {
      ...
      timers.add(Timer(const Duration(seconds: 8), () {
        pendingInvite = false;
    ...
    void onMatchOver() {
      stallTimer?.cancel(); // the match is done; never resign into a dead game
      if (winnerIsMe) {
        wins++;
      } else {
        losses++;
      ...
      // ignore: avoid_print
      print('MATCH OVER — winner: ${winnerIsMe ? "me" : "opponent"}  '
    ...
    timers.add(Timer(const Duration(minutes: 6), () {
    ...
    timers.add(Timer(const Duration(seconds: 3), () {
      lobbyReady = true;
      scheduleAct();
    }));
