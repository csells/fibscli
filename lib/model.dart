...
class GammonState extends ChangeNotifier {
  ...
  int _turnSign; // either -1, or 1, or 0 if no game is playing
  ...
  GammonState.clone(GammonState state)
      : points = List.from(state.points),
        ...
        _turnSign = state._turnSign;

  int get sideSign => _turnSign;
  ...
  void undo() {
    ...
    _turnSign = _undoState._turnSign;
    notifyListeners();
  }
  ...
  Iterable<GammonMove> getLegalMoves(int fromStartPip) sync* {
    ...
    if ((point < 1 ? -1 : 1) != _turnSign) return;
  ...
  int _sideSign(player1) => player1 ? -1 : 1;
  int _sideIndex(player1) => player1 ? 0 : 1;

  int pipCount({@required bool player1}) {
    final sign = _sideSign(player1);
    final index = _sideIndex(player1);
    var pipCount = 0;

    // pips left on the board
    for (var i = 0; i != 24; ++i) {
      final point = points[i];
      print('player1: $player1, point: $point');
      pipCount += point.sign != sign ? 0 : (sign == -1 ? -point * (i + 1) : point * (24 - i));
    }

    // pips left on the bar
    pipCount += _hits[index] * 24;
