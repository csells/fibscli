    ...
    if (_layouts == null) {
      ...
      for (var j = 0; j != 4; j++)
        for (var i = 0; i != 6; ++i) {
          ...
          // bottom-right
          if (pip >= 1 && pip <= 6) {
            layouts.add(PipLayout(pip: pip, left: 285 + dx, top: 249, labelDy: height));
          }
          // bottom-left
          else if (pip >= 7 && pip <= 12) {
            layouts.add(PipLayout(pip: pip, left: 21 + dx, top: 249, labelDy: height));
          }
          // top-left
          else if (pip >= 13 && pip <= 18) {
            layouts.add(PipLayout(pip: pip, left: 21 + dx, top: 21, labelDy: -labelHeight - 2));
          }
          // top-right
          else if (pip >= 19 && pip <= 24) {
            layouts.add(PipLayout(pip: pip, left: 285 + dx, top: 21, labelDy: -labelHeight - 2));
          }
          // error
          else {
