...
- **CI is LOCAL: `./tool/ci.sh`.** There is no GitHub Actions gate — the app's `gnubg_service` dependency is a pub **git dep on a private repo**, which a runner cannot clone without a long-lived token, so the gate lives on the developer machine that already has access. `tool/ci.sh` runs `flutter pub get`, `dart format --set-exit-if-changed .`, `dart analyze --fatal-infos .`, `flutter test`, then `tsc` + `vitest` for both Worker packages. It gates the **whole workspace** — `lib`/`test` **and** every `packages/` member, since those are first-party code we own outright (there is no upstream — see below). Nothing is excluded from the strict gate. A pre-push hook enforces it (`git config core.hooksPath .githooks`, once per clone; `git push --no-verify` bypasses deliberately). Run it before claiming a change is done; never push red. Dependabot still scans pub deps.
