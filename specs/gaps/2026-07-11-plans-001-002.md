...
| E3 | helper makes raw logging awkward (§7) | **partial** | logging is centralized + errors sanitized (`logging.ts:3-12`), but `logInfo/logWarn` accept free-form `Record<String, unknown>` — a raw frame is one natural call away; no typed constraint |
...
| F6 | analytics tests (§8) | **partial** | all six behaviors tested and passing, but dimension assertions are JSON-stringify containment, not per-column — a value in the wrong blob slot would still pass |
...
| H3 | CI runs worker checks (§10) | done | `ci.yml:21-29`; note: the repo's Dart CI job fails on the private `backgammon_ai` clone — unrelated to the worker job |
...
- Plan 001: **97.5%** = (38 + 0.5 × 2) / 40. No missing, no drifted.
- Plan 002: **100%** = 10 / 10.

## Drift

None found. The two partials are under-hardening relative to the spec's
letter, not contradictions of its intent.

## Outstanding items (ranked, ROI-filtered)

1. **E3 — typed logging metadata**: constrain `logInfo/logWarn` metadata to
   numbers/enums/booleans so a raw frame cannot be passed without a deliberate
   cast. Small change, real hardening of the privacy invariant.
2. **F6 — per-column analytics assertions**: assert blob-slot positions in the
   analytics tests instead of JSON-stringify containment. Test-quality only.
3. (dropped) B1 `/`-path upgrade test variant — the code path is shared and
   live-verified; a dedicated test adds near-zero information.
